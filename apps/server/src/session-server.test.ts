...
class InMemorySessionSnapshotDatabase implements SessionSnapshotDatabase {
  ...
  cloneRows(): Map<SessionId, SessionSnapshotRow> {
    return new Map(
      [...this.rows.entries()].map(([key, row]) => [key, this.clone(row)]),
    );
  }

  replaceRows(rows: Map<SessionId, SessionSnapshotRow>): void {
    this.rows.clear();
...
class InMemorySceneRecordDatabase implements SceneRecordDatabase {
  ...
  cloneRows(): Map<string, SceneRecordRow> {
    return new Map(
      [...this.rows.entries()].map(([sceneId, row]) => [
        sceneId,
        this.clone(row),
      ]),
    );
  ...
  replaceRows(rows: Map<string, SceneRecordRow>): void {
    this.rows.clear();

    for (const [sceneId, row] of rows.entries()) {
      this.rows.set(sceneId, this.clone(row));
    }
  }
...
function createSessionCommandTransactionHarness(
  ...
  return {
    sessionCommandTransaction: new DbBackedSessionCommandTransactionBoundary(
      unitOfWork,
      new CommandEventOutboxDispatcher(outboxDatabase, runtime.sessions),
    ),
    outboxDatabase,
...
async function setupDurableSessionForIdempotency(
  runtime: InMemoryGameRuntime<RuntimeCharacterRepository, RuntimeSessionStore>,
) {
  const session = await runtime.createSession({
    commandId: 'setup-session-create-session',
    type: 'create_session',
    actor: {
      participantId: 'dm-001',
      displayName: 'Dungeon Master',
  ...
  await runtime.joinSession({
    commandId: 'setup-session-join-player-1',
    type: 'join_session',
    actor: {
      participantId: 'player-001',
      displayName: 'Player One',
  ...
  const character = await runtime.createCharacter({
    commandId: 'setup-session-create-character-player-001',
    type: 'create_character',
    actor: {
      participantId: 'player-001',
    },
  ...
  const scene = await runtime.createScene({
    commandId: 'setup-session-create-scene',
    type: 'create_scene',
    actor: {
      participantId: 'dm-001',
    },
...
test('db-backed session transaction boundary writes one outbox row, dispatches after commit, and returns cached success on duplicate assign retry', async () => {
  ...
  const command = {
    commandId: 'transactional-assign-character-1',
    type: 'assign_character_to_participant',
    actor: {
      participantId: 'dm-001',
    },
  ...
  assert.equal(sessionUpdates[0]?.reason, 'participant_character_assigned');
  ...
  const assignedParticipant = sessionSnapshotRow?.snapshot.participants.find(
    (participant) => participant.id === 'player-001',
  );
...
test('db-backed session transaction boundary writes session and scene outbox rows, dispatches after commit, and returns cached success on duplicate activate retry', async () => {
  ...
  const command = {
    commandId: 'transactional-activate-scene-1',
    type: 'activate_scene_for_session',
    actor: {
      participantId: 'dm-001',
    },
  ...
  assert.equal(sessionSnapshotRow?.snapshot.session.activeSceneId, sceneId);
});

test('db-backed session transaction boundary persists create_session durably without inventing a session outbox publication path', async () => {
  const sessionDatabase = new InMemorySessionSnapshotDatabase();
  const characterDatabase = new InMemoryCharacterRecordDatabase();
  const idempotencyDatabase = new InMemoryCommandIdempotencyRecordDatabase();
  const outboxDatabase = new InMemoryCommandEventOutboxDatabase();
  ...
  const command = {
    commandId: 'transactional-create-session-1',
    type: 'create_session',
    actor: {
      participantId: 'dm-001',
      displayName: 'Dungeon Master',
  ...
  const first = await postJson<SessionCommandResponse>(
    runtime,
    idempotency,
    '/api/session/command',
    command,
  ...
  const second = await postJson<SessionCommandResponse>(
    runtime,
    idempotency,
    '/api/session/command',
    command,
  ...
  const persistedSnapshot = await sessionDatabase.getSessionSnapshot(
    first.body.data.sessionId,
  );

  assert.ok(persistedSnapshot);
  assert.equal(
    persistedSnapshot?.snapshot.session.id,
    first.body.data.sessionId,
  );
  assert.equal(persistedSnapshot?.snapshot.participants.length, 1);
  assert.equal(
    (await outboxDatabase.listUnpublishedCommandEventOutboxRecords()).length,
    0,
  );
...
test('db-backed session transaction boundary writes one outbox row for join_session, dispatches after commit, and returns cached success on duplicate retry', async () => {
  ...
  const created = await runtime.createSession({
    commandId: 'setup-transactional-join-create-session',
    type: 'create_session',
    actor: {
      participantId: 'dm-001',
      displayName: 'Dungeon Master',
  ...
  const commitMarkers: number[] = [];
  const updates = subscribeToSessionEvents(runtime, created.sessionId, () => {
    commitMarkers.push(unitOfWork.committedCount);
  });
  const command = {
    commandId: 'transactional-join-session-1',
    type: 'join_session',
    actor: {
      participantId: 'player-001',
      displayName: 'Player One',
  ...
  assert.equal(sessionUpdates.length, 1);
  assert.equal(sessionUpdates[0]?.type, 'session_state');
  assert.equal(sessionUpdates[0]?.reason, 'participant_joined');
  assert.deepEqual(newCommitMarkers, [commitCountBefore + 1]);
  assert.equal(
    (await outboxDatabase.listUnpublishedCommandEventOutboxRecords()).length,
    0,
  );
  assert.equal(sessionSnapshotRow?.snapshot.participants.length, 2);
});

test('db-backed session transaction boundary keeps no-op session mutations silent and does not write outbox rows', async () => {
  const sessionDatabase = new InMemorySessionSnapshotDatabase();
  const characterDatabase = new InMemoryCharacterRecordDatabase();
  const sceneDatabase = new InMemorySceneRecordDatabase();
  const idempotencyDatabase = new InMemoryCommandIdempotencyRecordDatabase();
  ...
  await runtime.activateSceneForSession({
    commandId: 'setup-session-noop-activate',
    type: 'activate_scene_for_session',
    actor: {
      participantId: 'dm-001',
    },
  ...
  const result = await postJson<SceneCommandResponse>(
    runtime,
    idempotency,
    '/api/scenes/command',
    {
      commandId: 'transactional-activate-scene-noop-1',
      type: 'activate_scene_for_session',
      actor: {
        participantId: 'dm-001',
      },
  ...
  assert.equal(idempotencyDatabase.recordCount, 1);
  assert.equal(outboxDatabase.recordCount, 0);
  assert.equal(updates.length, updateCountBefore);
  assert.equal(
    (await outboxDatabase.listUnpublishedCommandEventOutboxRecords()).length,
    0,
  );
...
test('db-backed scene transaction boundary commits create_scene durably and returns cached success on duplicate retry without inventing scene outbox rows', async () => {
  const sessionDatabase = new InMemorySessionSnapshotDatabase();
  const sceneDatabase = new InMemorySceneRecordDatabase();
  const characterDatabase = new InMemoryCharacterRecordDatabase();
  const idempotencyDatabase = new InMemoryCommandIdempotencyRecordDatabase();
  ...
  const createdSession = await runtime.createSession({
    commandId: 'transactional-scene-create-session-setup',
    type: 'create_session',
    actor: {
      participantId: 'dm-001',
      displayName: 'Dungeon Master',
  ...
  const command = {
    commandId: 'transactional-create-scene-durable-1',
    type: 'create_scene',
    actor: {
      participantId: 'dm-001',
    },
  ...
  assert.deepEqual(second.body, first.body);
  assert.equal(idempotencyDatabase.recordCount, 1);
  assert.equal(sceneDatabase.recordCount, 1);
  assert.equal(sceneRows.length, 1);
  assert.equal(outboxDatabase.recordCount, 0);

  if (!first.body.ok || !('scene' in first.body.data)) {
    return;
  }

  assert.equal(sceneRows[0]?.sceneId, first.body.data.scene.id);
...
test('db-backed scene transaction boundary commits place_entity_in_scene durably and returns cached success on duplicate retry without inventing scene outbox rows', async () => {
  ...
  const { sceneId, sessionId } =
    await setupDurableSessionForIdempotency(runtime);
  const command = {
    commandId: 'transactional-place-entity-durable-1',
...
test('move_character_in_active_scene falls back to the character transaction boundary when no combat boundary is injected', async () => {
  const characterDatabase = new InMemoryCharacterRecordDatabase();
  const encounterDatabase = new InMemoryActiveEncounterRecordDatabase();
  const idempotencyDatabase = new InMemoryCommandIdempotencyRecordDatabase();
  const outboxDatabase = new InMemoryCommandEventOutboxDatabase();
  ...
  await runtime.dmEndActiveEncounter({
    commandId: 'transactional-movement-character-fallback-end-encounter',
    type: 'dm_end_active_encounter',
    actor: {
      participantId: 'dm-001',
    },
  ...
  const commitCountBefore = unitOfWork.committedCount;
  const command = {
    commandId: 'transactional-movement-character-fallback-1',
    type: 'move_character_in_active_scene',
    actor: {
      participantId: 'player-001',
    },
