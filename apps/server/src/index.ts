...
server.listen(port, host, () => {
  console.log(
    `[server] Phase 12 DB idempotency claim plus scene transaction and session/character/movement/encounter/combat outbox foundation listening on http://${host}:${port}`,
  );

  if (persistenceMode === 'db') {
    console.log(
      '[server] Opt-in DB-backed startup is enabled via SERVER_PERSISTENCE_MODE=db; the runtime now injects the existing DB-backed character, session snapshot, scene, and active-encounter stores plus the current narrow transaction boundaries, pre-execution durable idempotency claims for covered DB-backed transactional mutations, covered DB-backed scene-only durable mutations, and covered session, movement, DM character update, encounter, and combat live-command post-commit outbox dispatch, but cold-boot unpublished rows are not auto-redelivered because SSE subscribers are process-local, without replay, multi-process coordination, or full combat continuity guarantees.',
    );
    return;
  }
