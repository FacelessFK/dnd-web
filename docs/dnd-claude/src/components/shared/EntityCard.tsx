...
export function EntityCard({ id, name, tagline, imageUrl, selected, onSelect }: Props) {
  return (
    <button
      onClick={() => onSelect(id)}
      ...
      style={{
        ...
        boxShadow: selected ? '0 0 20px rgba(201,168,76,0.25)' : '0 1px 4px rgba(0,0,0,0.2)',
      }}
    >
      <div
        className="w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 border"
        style={{ borderColor: selected ? 'var(--color-gold)' : 'var(--color-border)', background: 'var(--color-surface-elevated)' }}
      >
        <img
          src={imageUrl}
          alt={name}
      ...
      <div className="flex-1 min-w-0 text-center">
        ...
        <div className="text-[11px] mt-0.5 line-clamp-2" style={{ color: 'var(--color-text-muted)' }}>
          {tagline}
        </div>
      </div>
    </button>
