...
- Use `dnd-feature-factory` to orchestrate a small Training Room Skirmish /
  Phase 6-style polish slice, or a similarly narrow approved task, through the
  fuller role-skill workflow: optional `dnd-codebase-researcher`,
  `dnd-story-writer`, human story approval, `dnd-spec-writer`, human spec
  approval, `dnd-runtime-boundary-review` when risky,
  `dnd-build-with-tests`, `dnd-test-verifier`,
  `dnd-implementation-validator`, `dnd-pr-reviewer`, and a human merge
  decision. It may use a lightweight path for tiny docs-only or simple UI
  polish tasks, but it must not skip human approval before implementation or
  boundary review for risky runtime/product-scope changes. This skill is
  workflow-only: it must not broaden scope into CRPG/full automation/monster
  AI/full spell systems/fog of war/production auth, or bypass
  DM/server-authoritative boundaries.
